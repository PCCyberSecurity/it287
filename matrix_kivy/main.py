...
log_file_path = os.path.expandvars("%APPDATA%\matrix_scr")
...
handler = RotatingFileHandler(filename=os.path.join(log_file_path, 'matrix.log'), maxBytes=2048000, backupCount=2, encoding="utf-8")
handler.setLevel(logging.DEBUG)
logger.addHandler(handler)

#logging.basicConfig(filename=os.path.join(log_file_path, 'matrix.log'), encoding="utf-8", level=logging.DEBUG)


# Set path for nuitka to find resources
is_nuitka = "__compiled__" in globals()
if is_nuitka:
    logger.info("Running Compiled...")
...
#Config.set('graphics', 'maxfps', 35)  # 0 for unlimited fps

from kivy.app import App
from kivy.core.window import Window
from kivy.uix.widget import Widget
from kivy.uix.label import Label
from kivy.graphics import Color, Ellipse, Line, Rectangle, RoundedRectangle, BindTexture
...
def get_desktop_size(multi_monitor=True):
    import ctypes
...
class MainWidget(Widget):
    ...
    _ROOT_WIDGET = None
    # Render falling lines/tails here
    _FALLING_LINES_WIDGET = None
    ...
    def __init__(self, **kwargs):
        ...
        if MainWidget._FALLING_LINES_WIDGET is None:
            MainWidget._FALLING_LINES_WIDGET = Widget()
            MainWidget._FALLING_LINES_WIDGET.size=self.size
            MainWidget._FALLING_LINES_WIDGET.pos=self.pos
            # Add at a higher index so it will draw first
            self.add_widget(MainWidget._FALLING_LINES_WIDGET, index=10)
            # Setup the drawing instructions for our widget
            with MainWidget._FALLING_LINES_WIDGET.canvas.before:
        ...
        if screen_saver_param == "p":
            ...
            #logger.debug(f"{self.window_pos} {self.window_size}")
        ...
        # Add the clock
        self._clock_control = ClockDisplayLabel()
        self.add_widget(self._clock_control, index=0)

        # 0 - Run every frame, 0.042 = 1/24 - 24 frames per second - only call the step event at a certain fps seperate from the render fps
        Clock.schedule_interval(self._on_step, 0.042) # 0)
        Clock._max_fps = 35
    ...
    def _on_mouse_pos(self, instance, pos):
        ...
        if moved:
            ...
            if a:
                logger.info("Closing - mouse moved.")
                a.stop()
    ...
    def _on_key_down(self, keyboard, keycode, text, modifiers):
        ...
        if keycode[1] == "f":
            ...
            Clock._max_fps += 10
            if Clock._max_fps > 120:
                Clock._max_fps = 120
        elif keycode[1] == '-':
            Clock._max_fps -= 10
            if Clock._max_fps < 24:
                Clock._max_fps = 24
    ...
    def resized(self, instance, width, height):
        ...
        # logger.debug(f"resized: {instance.width} -> {width} ")
        self.width = width
        self.height = height
        if not MainWidget._FALLING_LINES_WIDGET is None:
            MainWidget._FALLING_LINES_WIDGET.width = width
            MainWidget._FALLING_LINES_WIDGET.height = height
        self.add_falling_lines()
    ...
    def add_falling_lines(self):
        ...
        #logger.info(f"AddFallingLins: {num_lines}")
...
class FPSControl(object):
    def __init__(self):
        ...
        self._canvas_background_texture = RoundedRectangle(
            pos=self.pos, size=self._fps_label.texture.size,
            radius=[(10, 10)],

        )
        # Add to canvas drawing list
        MainWidget._FALLING_LINES_WIDGET.canvas.after.add(self._canvas_background_color)
        MainWidget._FALLING_LINES_WIDGET.canvas.after.add(self._canvas_background_texture)
        MainWidget._FALLING_LINES_WIDGET.canvas.after.add(self._canvas_text_color)
        MainWidget._FALLING_LINES_WIDGET.canvas.after.add(self._canvas_fps_text)
...
class ClockDisplayLabel(Label):
    def __init__(self, **kwargs):

        # Make a custom rendercontext so we can set shaders
        ...
        super(ClockDisplayLabel, self).__init__(**kwargs)
        ...
        self.halign = 'center'
        self.valign = 'middle'
        self.desktop_size = get_desktop_size(multi_monitor=False)

        self.border = 10

        # Position in the upper middle of the screen
        #self.size = (50, 50)
        self.pos = (self.desktop_size[0] / 2 - self.size[0] / 2, self.desktop_size[1] - self.size[1] - self.border)
        ...
        # Add to canvas drawing list
        self.canvas.before.add(self._canvas_background_color)
        self.canvas.before.add(self._canvas_background_texture)


        # Add shadow label for dropshadow
        self._shadow_label = Label(markup=True)
        self._shadow_label.halign = self.halign
        self._shadow_label.valign = self.valign

        self._shadow_label.text = self.text
        #self._shadow_label.color = (0.2, 0.2, 0.2, 0.8)
        self._shadow_label.pos = (self.pos[0] + 2, self.pos[1] - 2)
        self._shadow_label.texture_update()

        self._shadow_label_render = Rectangle(
            pos = self._shadow_label.pos, size=self._shadow_label.size, texture=self._shadow_label.texture
        )
        self.canvas.before.add(Color(.1, .1, .1, 1.0))
        self.canvas.before.add(self._shadow_label_render)
    ...
    def update_clock(self):
        if not self.visible:
            # Hiden, use an empty texture
            #self._canvas_background_texture = None
            return

        date_text = datetime.datetime.now().strftime("%a %b %d, %Y")
        time_text = datetime.datetime.now().strftime("%I:%M:%S %p")

        # max_width = len(date_text) + 6
        # if len(time_text) + 6 > max_width:
        #     max_width = len(time_text) + 6
        # date_text = date_text.center(max_width)
        # time_text = time_text.center(max_width)
        clock_text = f"[b][color=#ffffffff][size=64]{time_text}[/size][/color]\n[size=24]{date_text}[/size][/b]"
        
        # Redraw the label and set the texture/pos so it gets rendered
        self.text = clock_text
        #self._clock_label.refresh()
        self.texture_update()
        self.size = self.texture_size
        self.pos = (self.desktop_size[0] / 2 - self.size[0] / 2, self.desktop_size[1] - self.size[1] - self.border - self.border)

        # Update shadow label
        self._shadow_label.text = self.text.replace("#ffffffff", "#000000aa")
        self._shadow_label.opacity = 1.0
        self._shadow_label.pos = (self.pos[0] + 3, self.pos[1] - 3)
        #self._shadow_label.color = (1, 1, 1, 1)
        self._shadow_label.texture_update()

        self._shadow_label_render.texture = self._shadow_label.texture
        self._shadow_label_render.size = self.size
        self._shadow_label_render.pos = self._shadow_label.pos
        
        # Size should be text size + border * 2
        #self.size = (self._clock_label.texture.size[0] + self.border + self.border, self._clock_label.texture.size[1] + self.border + self.border)

        # Position in the upper (minus the border) middle of the primary screen
        #self.pos = (self.desktop_size[0] / 2 - self.size[0] / 2 , self.desktop_size[1] - self.size[1] - self.border)

        self._canvas_background_texture.size = (
            self.size[0] + self.border + self.border,
            self.size[1] + self.border + self.border
        )
        self._canvas_background_texture.pos = (
...
class FallingLine(object):
    ...
    def __init__(self, text="", x=0, y=0, **kwargs):
        ...
        MainWidget._FALLING_LINES_WIDGET.canvas.add(self._canvas_text_color)
        MainWidget._FALLING_LINES_WIDGET.canvas.add(self._canvas_text_instruction)

        # Catch frame update event
        MainWidget._ROOT_WIDGET.bind(on_step=self.on_step)

        self.reset_item()
        # logger.debug(f"NEW {self.pos}")
    ...
    def drop_tail_character(self):
        ...
        if not MainWidget._FALLING_LINES_WIDGET.collide_point(self.x, self.y):
    ...
    def destroy(self):
        ...
        MainWidget._FALLING_LINES_WIDGET.canvas.remove(self._canvas_text_color)
        MainWidget._FALLING_LINES_WIDGET.canvas.remove(self._canvas_text_instruction)
...
def main():
    ...
    logger.debug("Creating Kivy App.")
    app = MatrixApp()
    app.run()
    logger.debug("Exiting Kivy App.")
